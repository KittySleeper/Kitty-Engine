...
import kitty.objects.NoteSplash;
import kitty.objects.HealthIcon;
import kitty.objects.BackgroundGirls;
import kitty.shaders.WiggleEffect;
import kitty.backend.modding.scripting.HScript;
import kitty.backend.modding.scripting.ModchartState;
import kitty.backend.Ratings;
import kitty.backend.EtternaFunctions;
import kitty.backend.ConvertScore;
import kitty.backend.HelperFunctions;
import kitty.states.menus.options.OptionsMenu;
import kitty.states.menus.*;
import kitty.states.debug.AnimationDebug;
import kitty.states.editors.ChartingState;
...
class PlayState extends MusicBeatState
{
	...
	public static var songVariant:String = null;
	...
	public var keys = [false, false, false, false];
	...
	public var splashySplashys:FlxTypedGroup<NoteSplash> = null;
	...
	override public function create()
	{
		...
		var stageCheck:String = SONG.stage == null ? 'mainStage' : SONG.stage;

		// script creation and execution

		var stageScript = new HScript('data/stages/$stageCheck');
		if (!stageScript.isBlank && stageScript.expr != null)
		{
			stageScript.interp.scriptObject = this;
			stageScript.setValue('add', add);
			stageScript.setValue('remove', remove);
			stageScript.interp.execute(stageScript.expr);
		}
		scripts.push(stageScript);

		for (epicSongScript in Paths.readDirectory('data/songs/$songLowercase/scripts'))
		{
			var songScript = new HScript('data/songs/$songLowercase/scripts/$epicSongScript');
			if (!songScript.isBlank && songScript.expr != null)
			{
				songScript.interp.scriptObject = this;
		...
		if (!PlayStateChangeables.Optimize)
		{
			...
			splashySplashys = new FlxTypedGroup<NoteSplash>();
			add(splashySplashys);
			...
			kadeEngineWatermark = new FlxText(4, healthBarBG.y
				...
				SONG.song
				+ " - "
				+ CoolUtil.toTitleCase(storyDifficulty)
				+ " - Kitty Engine V:"
				+ MainMenuState.kittyEngineVer, 16);
			...
			scoreTxt = new FlxText(0, healthBarBG.y + 35, FlxG.width, "");
			scoreTxt.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
			add(scoreTxt);

			botPlayState = new FlxText(0, healthBarBG.y + (PlayStateChangeables.useDownscroll ? 35 : -35), FlxG.width, loadRep ? "REPLAY" : "BOTPLAY", 20);
			botPlayState.setFormat(Paths.font("vcr.ttf"), 35, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
			...
			splashySplashys.cameras = [camHUD];
	...
	function startCountdown():Void
	{
		...
		startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
		{
			...
			for (script in scripts)
				script.callFunction('countdownEvent', [swagCounter, introSpr]);

			swagCounter += 1;
		}, 5);

		for (script in scripts)
			script.callFunction('postStartCountdown');
	}
	...
	{ // this actually handles press inputs
		...
		else if (!KadeEngineData.kittyOptions.get("ghost_tap") && songStarted)
		{
			noteMiss(data, null);
			ana.hit = false;
			ana.hitJudge = "shit";
	...
	private function generateSong(dataPath:String):Void
	{
		...
		FlxG.sound.playMusic(Paths.inst(PlayState.SONG.song, songVariant), 1, false);
		...
		if (SONG.needsVoices)
		{
			...
			{
				var voices = new FlxSound().loadEmbedded(Paths.voices(PlayState.SONG.song, null, songVariant));
		...
		for (section in noteData)
		{
			...
			for (songNotes in section.sectionNotes)
			{
				...
				for (susNote in 0...Math.floor(susLength))
				{
					...
					sustainNote.x = swagNote.x + ((swagNote.frameWidth - sustainNote.frameWidth) / 2);
				}
		...
		// Link sustain notes so each note knows the next segment in its hold chain
		if (unspawnNotes.length > 1)
		{
			for (i in 0...unspawnNotes.length - 1)
			{
				var a:Note = unspawnNotes[i];
				var b:Note = unspawnNotes[i + 1];
				if (a.noteData == b.noteData && a.isSustainNote)
					a.nextNote = b;
			}
		}
	...
	override public function update(elapsed:Float)
	{
		...
		for (icon in [iconP1, iconP2])
			icon.scale.x = icon.scale.y = FlxMath.lerp(icon.scale.x, 1, 0.15);
		...
		// {
		// 	var offsetX = dad.camPositionOffset[0];
		// 	var offsetY = dad.camPositionOffset[1];
		...
		// 	&& PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection)
		// {
		// 	var offsetX = boyfriend.camPositionOffset[0];
		// 	var offsetY = boyfriend.camPositionOffset[1];
		// 	#if windows
		// 	if (luaModchart != null)
		// 	{
		// 		offsetX = luaModchart.getVar("followXOffset", "float");
		...
		// 	#end
		// }
		// old camera system, make sure to backport this at some point
		...
		if (generatedMusic)
		{
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (!daNote.mustPress && daNote.wasGoodHit)
				{
					...
					for (script in scripts)
						script.callFunction('opponentNoteHit', [daNote]);
					daNote.script.callFunction('opponentNoteHit');
				}
			...
			for (event in events)
			{
				...
				{
					events.remove(event);
					executeEvent(event.name, event.params);
				}
			}
		}
		...
		else
		{
			if (isStoryMode)
			{
				...
				else
				{
					...
					PlayState.SONG = Song.loadFromJson(songFormat, storyDifficulty);
					FlxG.sound.music.stop();
	...
	private function keyShit():Void // I've invested in emma stocks
	{
		...
		if (holdArray.contains(true) && generatedMusic)
		{
			notes.forEachAlive(function(daNote:Note)
			{
				if (daNote.isSustainNote && daNote.canBeHit && daNote.mustPress && holdArray[daNote.noteData] && daNote.overlaps(strumLine))
				{
					goodNoteHit(daNote);
				}
			});
		...
		if ((KeyBinds.gamepad && !FlxG.keys.justPressed.ANY))
		{
			...
			if (pressArray.contains(true) && generatedMusic)
			{
				...
				{
					if (!KadeEngineData.kittyOptions.get("ghost_tap"))
					{
				...
				else if (!KadeEngineData.kittyOptions.get("ghost_tap"))
				{
					for (shit in 0...pressArray.length)
						if (pressArray[shit])
							noteMiss(shit, null);
		...
		if (PlayStateChangeables.botPlay)
		{
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (daNote.overlaps(strumLine) && daNote.mustPress && daNote.rating == "sick" && !loadRep)
				{
					goodNoteHit(daNote);
					...
					playerStrums.forEach(function(spr:FlxSprite)
					{
						...
						spr.animation.onFinish.add((anim) ->
						{
				...
				if (loadRep && findByTime(daNote.strumTime) != null && daNote.mustPress)
				{
					goodNoteHit(daNote);
	...
	function noteMiss(direction:Int = 1, daNote:Note):Void
	{
		...
		for (script in scripts)
			script.callFunction('noteMiss', [direction, daNote]);
	}
	...
	function goodNoteHit(note:Note):Void
	{
		...
		if (!note.wasGoodHit)
		{
			...
			playerStrums.forEach(function(spr:FlxSprite)
			{
				if (Math.abs(note.noteData) == spr.ID)
				{
					...
					var splashySplash:NoteSplash = new NoteSplash(0, 0, note);
					splashySplash.setPosition(spr.x + ((spr.width - splashySplash.width) / 2), spr.y + ((spr.height - splashySplash.height) / 2));
					if (note.isSustainNote)
					{
						splashySplash.x -= 15;
						splashySplash.y += 45;
					...
					splashySplashys.add(splashySplash);
				}
			});
		...
		for (script in scripts)
		{
			...
			script.callFunction('playerNoteHit', [note]);
		}

		note.script.callFunction('goodNoteHit');
		note.script.callFunction('playerNoteHit');
	}

	// most of these events were taken mostly from https://github.com/CodenameCrew/CodenameEngine/blob/cd7c9f1afce8a938f7ea7962dafbcca6e2221e78/source/funkin/game/PlayState.hx#L1529
	...
	public function executeEvent(eventName:String, params:Array<Dynamic>)
	{
		...
		switch (eventName)
		{
			case "Camera Modulo Change":
				camZoomingInterval = params[0];
				camZoomingStrength = params[1];

			case "Camera Zoom":
				var cam = params[2] == "camHUD" ? camHUD : camGame;
				var name = (params[2] == "camHUD" ? "camHUD" : "camGame") + ".zoom";
				var tween = eventsTween.get(name);
				if (tween != null)
					tween.cancel();

				var finalZoom:Float = params[1] * (params[6] == 'direct' ? defaultCamZoom : stageDefaultCamZoom);
				if (params[7] == true)
					finalZoom *= cam.zoom;
				...
				if (params[0] == false)
				{
					...
					eventsTween.set(name, FlxTween.tween(cam, {zoom: finalZoom}, (Conductor.stepCrochet / 1000) * params[3], {
			...
			case "Camera Movement":
				...
				if (params[3] == "CLASSIC")
				{
					...
					{
						case 0:
							var offsetX = dad.camPositionOffset[0] + dad.getMidpoint().x + 150;
							var offsetY = dad.camPositionOffset[1] + dad.getMidpoint().y - 100;
							eventsTween.set("cameraMovement",
								FlxTween.tween(camFollow, {x: offsetX, y: offsetY}, (Conductor.stepCrochet / 1000) * params[2], {ease: FlxEase.expoOut}));

						case 1:
							var offsetX = boyfriend.camPositionOffset[0] + boyfriend.getMidpoint().x - 100;
							var offsetY = boyfriend.camPositionOffset[1] + boyfriend.getMidpoint().y - 100;
							eventsTween.set("cameraMovement",
								FlxTween.tween(camFollow, {x: offsetX, y: offsetY}, (Conductor.stepCrochet / 1000) * params[2], {ease: FlxEase.expoOut}));

						case 2:
							var offsetX = gf.camPositionOffset[0] + gf.getMidpoint().x;
							var offsetY = gf.camPositionOffset[1] + gf.getMidpoint().y;
							eventsTween.set("cameraMovement",
								FlxTween.tween(camFollow, {x: offsetX, y: offsetY}, (Conductor.stepCrochet / 1000) * params[2], {ease: FlxEase.expoOut}));
					}
				}

				cameraFocus = params[0] == null ? 0 : params[0];
			...
			case "Camera Position":
				...
				{
					case 0:
						offsetX = dad.camPositionOffset[0] + dad.getMidpoint().x + 150;
						offsetY = dad.camPositionOffset[1] + dad.getMidpoint().y - 100;

					case 1:
						offsetX = boyfriend.camPositionOffset[0] + boyfriend.getMidpoint().x - 100;
						offsetY = boyfriend.camPositionOffset[1] + boyfriend.getMidpoint().y - 100;

					case 2:
						offsetX = gf.camPositionOffset[0] + gf.getMidpoint().x;
						offsetY = gf.camPositionOffset[1] + gf.getMidpoint().y;
				}

				var isOffset = params[6] == true;
				if (params[2] == false)
				{
					camFollow.setPosition(isOffset ? (offsetX + params[0]) : params[0], isOffset ? (offsetY + params[1]) : params[1]);
				}
				else
				{
					if (params[4] != null)
					{
						eventsTween.set("cameraMovement",
							FlxTween.tween(camFollow, {x: isOffset ? (offsetX + params[0]) : params[0], y: isOffset ? (offsetY + params[1]) : params[1]},
								(Conductor.stepCrochet / 1000) * (params[3] == null ? 4 : params[3]), {
									ease: params[4] == "CLASSIC" ? FlxEase.expoOut : CoolUtil.flxeaseFromString(params[4], params[5])
								}));
					}
				}
		}
